...
func (c *Character) clampHPDebt() {
	...
	if c.currentHPDebt < 0 {
		c.currentHPDebt = 0
	} else if c.currentHPDebt > maxhp*2 {
		c.currentHPDebt = maxhp * 2
	}
}
...
func (c *Character) ModifyHPDebtByAmount(amt float64) {
	...
	c.core.Events().HPDebt.Emit(event.HPDebtEvent{
		CharIndex: c.Index,
		Amount:    prevHPDebt - c.currentHPDebt,
	})
}
