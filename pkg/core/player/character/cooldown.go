...
//   - a info.Action: this is the action type we are triggering the cooldown for
...
func (c *Character) SetCD(a info.Action, dur int) {
	...
	modified := int(float64(dur) * (1 + c.Stat(attributes.CooldownR)))
...
func (c *Character) cdQueueString(a info.Action) string {
...
func (c *Character) SetNumCharges(a info.Action, num int) {
	c.additionalCDCharge[a] = num - 1
	c.AvailableCDCharge[a] = num
}

func (c *Character) Charges(a info.Action) int {
	return c.AvailableCDCharge[a]
}

func (c *Character) SetCDWithDelay(a info.Action, dur, delay int) {
	if delay == 0 {
		c.SetCD(a, dur)
		return
	}
...
func (c *Character) Cooldown(a info.Action) int {
...
func (c *Character) ResetActionCooldown(a info.Action) {
...
func (c *Character) ReduceActionCooldown(a info.Action, v int) {
...
func (c *Character) startCooldownQueueWorker(a info.Action) {
