...
func (r *Reactable) TryVaporize(a *info.AttackEvent) bool {
	...
	case attributes.ElementFire:
		...
		consumed = r.reduce(attributes.ElementWater, a.Info.Durability, .5)
		a.Info.AmpMult = 1.5
	case attributes.ElementWater:
		...
		consumed = r.reduce(attributes.ElementFire, a.Info.Durability, 2)
		a.Info.AmpMult = 2
		r.burningCheck()
	default:
		// should be here
	...
	r.core.Events().Vaporize.Emit(event.ReactionEvent{
		Target:      r.self.Key(),
		AttackEvent: a,
	})
	return true
